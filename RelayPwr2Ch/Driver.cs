...
{
    //
    // Your driver's DeviceID is ASCOM.RelayPwr2Ch.Switch
    //
    // The Guid attribute sets the CLSID for ASCOM.RelayPwr2Ch.Switch
    // The ClassInterface/None addribute prevents an empty interface called
    // _RelayPwr2Ch from being created and used as the [default] interface
 

    /// <summary>
    /// ASCOM Switch Driver for RelayPwr2Ch.
    /// </summary>
    [Guid("b344b3c1-300e-4d83-9b9d-c18a3307f479")]
    ...
    public class Switch : ISwitchV2
    {
        ...
        /// </summary>
        internal static string driverID = "ASCOM.RelayPwr2Ch.Switch";
    
        /// <summary>
        /// Driver description that displays in the ASCOM Chooser.
        /// </summary>
        private static string driverDescription = "ASCOM Switch Driver for RelayPwr2Ch.";
        ...
        string relay0, relay1; // names for the relays
        string[] configure = new string[2]; // for relay names
        // for serial commands
        internal byte startByte = 0xA0;  // start character of relay command
        private byte[] relayStatus = new byte[2];  //  xx xx xx xx  01 /00 for each relay

        internal byte[] command = new byte[4];  // relay command
        internal byte[] relayRelay = new byte[] { 0xFF };
        byte setpin = 0x01, clearpin = 0x00;  // control bytes to set and reset relay

        String path = Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments) + "\\ASCOM\\RelayPwr2Ch";
        /// <summary>
        ...
        private Utilities.Serial Serial2;
        /// <summary>
        ...
        {
            Serial2.Speed = Utilities.SerialSpeed.ps9600;
            Serial2.PortName = comPort;
            Serial2.Parity = Utilities.SerialParity.None;
            Serial2.DataBits = 8;
            Serial2.StopBits = Utilities.SerialStopBits.One;
            Serial2.Handshake = Utilities.SerialHandshake.None;
            Serial2.ReceiveTimeoutMs = 100;

            try
            {
                Serial2.Connected = true;
            }
            catch (Exception)
            {
        ...
        public bool Connected
        {
            ...
            set
            {
                ...
                {
                    connectedState = true;
                    tl.LogMessage("Connected Set", string.Format("Connecting to port {0}", comPort));
                    SetupSwitch();
                }
                else
                {
                    connectedState = false;
                    tl.LogMessage("Connected Set", string.Format("Disconnecting to port {0}", comPort));
                    Serial2.Dispose();
                }
            }
        }
        ...
        public bool GetSwitch(short id)
        {
            ...
            try
            {
                Serial2.ClearBuffers();
                Serial2.TransmitBinary(relayRelay);

                for (int i = 0; i < numSwitch; i++)
                {
                    string val = Serial2.ReceiveTerminated(Environment.NewLine);
                    tl.LogMessage("incoming - ", val);
                    if (val.Contains("ON")) { relayStatus[i] = 0x01; }
                    else { relayStatus[i] = 0x00; }
            ...
            {
                tl.LogMessage("GetSwitch", string.Format("{0}\n", e));
        ...
        public void SetSwitch(short id, bool state)
        {
            ...
            else
            {
                ...
                relaycmd[0] = 0xA0;
                relaycmd[1] = (byte)(id + 1);
                if (state)
                {
                    relaycmd[2] = (byte)(setpin);
                    relaycmd[3] = (byte)(162 + id);
                }
                else
                {
                    relaycmd[2] = (byte)(clearpin);
                    relaycmd[3] = (byte)(161 + id);
                }
                tl.LogMessage("SetSwitch - sending this: ", string.Format("{0},{1},{2},{3}", relaycmd[0], relaycmd[1], relaycmd[2], relaycmd[3]));
                Serial2.ClearBuffers();
                Serial2.TransmitBinary(relaycmd);
            }
        }
        ...
        private void filewrite()
        {
            try
            {
                ...
                File.WriteAllLines(path + "\\relay2Chsettings.txt", configure);
            }
        ...
        // fileread() reads configuration variables from MyDocuments/ASCOM/RelayPwr/relaysettings.txt
        private void fileread()
        {
            try
            {
                configure = File.ReadAllLines(path + "\\relay2Chsettings.txt");
                relay0 = configure[0];
                relay1 = configure[1];
